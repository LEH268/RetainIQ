        ...
        <table className="w-full text-sm text-left">           
          <thead>             
            <tr className="border-b border-[var(--color-border)] bg-gray-50 text-xs uppercase tracking-wider text-ink/50 font-bold">               
              <th className="px-6 py-4">Customer</th> <th className="px-6 py-4">Status</th> <th className="px-6 py-4">Segment</th> <th className="px-6 py-4">Health Score</th>               
              <th className="px-6 py-4">Churn Prob.</th> <th className="px-6 py-4">Risk Level</th> <th className="px-6 py-4 text-right">Action</th>             
            </tr>           
          </thead>           
          <tbody className="divide-y divide-[var(--color-border)]">             
            {filtered.map((c) => (               
              <tr key={c.id} className="hover:bg-blue-50/30 transition-colors group">                 
                ...
                <td className="px-6 py-4">                   
                  <span className={`px-2.5 py-1 text-xs font-bold rounded-md border ${c.status === 'Active' ? 'bg-green-50 text-green-700 border-green-200' : c.status === 'New' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>{c.status}</span>                 
                </td>                 
                ...
                <td className="px-6 py-4 text-right">                   
                  <Link to={`/customers/${c.id}`} className="inline-block px-4 py-2 text-xs font-bold bg-[var(--color-brand-soft)] text-[var(--color-brand)] rounded-xl hover:bg-[var(--color-brand)] hover:text-white transition-all">Analyze AI</Link>                 
                </td>               
              </tr>             
            ))}             
            {filtered.length === 0 && (               
